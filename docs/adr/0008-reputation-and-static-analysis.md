...
2.  **Pre-Execution Static Analysis**: Integrating `bandit` for Python payloads in `apple_sandbox.py`.
    - Any code payload attempting `os.system`, `eval`, or using hardcoded secrets will be rejected *before* sandbox entry.
3.  **Semantic Alignment (Planned)**: Detecting drift between an agent's technical intent and its execution pattern.
