...
- [ ] **[P1] [SECURITY] Fail-Closed PQC Policy**: Modify `hybrid.py` to reject any ActionRecord that lacks a valid `ML-DSA-65` signature (strip-attack protection).
- [ ] **[P1] [SECURITY] Model Integrity Warden**: Nightly cryptographic hashing of `mlx_lm` weights signed by the Root PQC identity.
- [ ] **[P2] [LEARNING] LoRA Fine-Tuning Loop**: Daily local model updates derived from human-approved Airlock outcomes to internalize operator policy.
