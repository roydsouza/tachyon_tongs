...
#### Phase 24.0 — Core CLI Skeleton [DONE]
- [x] **[SCAFFOLD]** Create `tachyon/cli/__init__.py` + `main.py` (Typer entrypoint). [DONE]
- [x] **[PYPROJECT]** Register `tt` console script in `pyproject.toml`. [DONE]
- [x] **[REFACTOR]** Extract `scripts/airlock_cli.py` → `tachyon/cli/airlock.py` library. [DONE]
- [x] **[CMD]** Implement `tt status`, `tt health`, `tt verify` commands. [DONE]
- [x] **[CMD]** Implement `tt agent list|run|stop|kill|restart|inspect|tail` commands. [DONE]
- [x] **[CMD]** Implement `tt airlock list|show|approve|deny|test` commands. [DONE]
- [x] **[CMD]** Implement `tt logs tail|search|export` commands. [DONE]
- [x] **[CMD]** Implement `tt catalog search|show|export` commands. [DONE]
- [x] **[OUTPUT]** Add `--json` flag for machine-readable output on all commands. [DONE]

#### Phase 24.1 — Textual TUI Dashboard [DONE]
- [x] **[APP]** Create `tachyon/cli/tui/app.py` (Textual app shell). [DONE]
- [x] **[WIDGET]** Substrate Health manifold (status, uptime, integrity, Merkle root). [DONE]
- [x] **[WIDGET]** Active Agents manifold (status table with live PID/CPU/memory). [DONE]
- [x] **[WIDGET]** Recent Activity manifold (scrolling event feed). [DONE]
- [x] **[WIDGET]** Airlock Queue manifold (pending patches with debate status). [DONE]
- [x] **[WIDGET]** Log Streaming manifold (filterable, follow mode, regex search). [DONE]
- [x] **[NAV]** Vi-style keybindings (`j/k`, `Ctrl+d/u`, `:`, `/`). [DONE]
- [x] **[WS]** WebSocket client for real-time agent state + log streaming. [DONE]
- [x] **[REUSE]** Integrate existing `airlock_tui.py` widgets where applicable. [DONE]

#### Phase 24.2 — NeoVIM Plugin (`tachyon.nvim`) [DONE]
- [x] **[SKELETON]** Create `plugin/tachyon.nvim/` directory with Lua structure. [DONE]
- [x] **[API]** Implement HTTP API client in Lua (via `curl`). [DONE]
- [x] **[UI]** Floating dashboard window (`:TachyonDash`). [DONE]
- [x] **[UI]** Airlock 3-way split review (`:TachyonAirlock`). [DONE]
- [x] **[UI]** Live log buffer with async job (`:TachyonLogs`). [DONE]
- [x] **[TELESCOPE]** Register Telescope pickers (agents, debates, catalog). [DONE]
- [x] **[LSP]** Rego LSP configuration via `lspconfig`. [DONE]
- [x] **[SYNTAX]** Custom filetype detection (`.sig`, `SKILL.md`, debate transcripts). [DONE]
- [x] **[SYNTAX]** Debate transcript syntax highlighting (`syntax/debate.vim`). [DONE]
- [ ] **[DOCS]** Write `:help tachyon` Vim documentation.
- [x] **[KEYBINDS]** Map `<leader>t*` keybindings (dashboard, agents, airlock, logs, catalog). [DONE]
