...
1.  **Format Adherence:** When a valid threat is found, append it to `exploits/CATALOG.md`. Follow the existing format (Attack ID, Status, Description, Payload, Expected Defense, Mitigation).
...
3.  **Propose Mitigation:** If a mitigation does not currently exist in the Tachyon Tongs pipeline, generate an actionable ticket and append it to `tasks/TASKS.md` for human review.

## Phase 4: Commit and Sleep
// turbo-all
1.  Run `git add exploits/CATALOG.md tasks/TASKS.md`
