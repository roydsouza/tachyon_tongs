...
class TestCLIArgParsing(unittest.TestCase):
    ...
    def test_tt_help(self):
        ...
        result = subprocess.run(["tt", "--help"], capture_stdout=True, text=True)
    ...
    def test_tt_keys_help(self):
        ...
        result = subprocess.run(["tt", "keys", "--help"], capture_stdout=True, text=True)
        self.assertEqual(result.returncode, 0)
    ...
    def test_cmd_routing(self):
        ...
        # Testing Typer-style commands (no flags needed for subcommands)
        result = subprocess.run(["tt", "keys"], capture_stdout=True, text=True)
        self.assertEqual(result.returncode, 0)
        self.assertIn("genesis", result.stdout)
