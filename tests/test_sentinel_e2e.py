...
class TestSentinelE2E(unittest.TestCase):
    ...
    def test_sentinel_runs_from_cli_without_fatal_errors(self, mock_post):
        ...
        result = subprocess.run(
            [sys.executable, "scripts/sentinel.py", "--manual", "--log-file", self.test_log],
            capture_output=True,
            text=True,
            env=self.env
        )
    ...
    def test_sentinel_prepending_works(self, mock_post):
        ...
        mock_post.return_value.json.return_value = {"result": True}
        # Run once
        subprocess.run([sys.executable, "scripts/sentinel.py", "--manual", "--log-file", self.test_log], env=self.env)
        time.sleep(1.1)
        # Run again with a different source
        subprocess.run([sys.executable, "scripts/sentinel.py", "--cron", "--log-file", self.test_log], env=self.env)
